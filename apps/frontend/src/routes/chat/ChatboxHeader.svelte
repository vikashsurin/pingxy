<script lang="ts">
    ...
    const username = $derived(chatStore.activeConversation?.user?.username);
    const gender = $derived(chatStore.activeConversation?.user?.data.gender);
    const uid = $derived(chatStore.activeConversation?.user?.id);
    const country = $derived(chatStore.activeConversation?.user?.data.country);
    let toggleMenu = $state(false);
</script>

<div class="flex relative bg-gray-200 py-1 px-2 shrink-0 text-sm">
    {#if chatStore.activeConversation?.user}
