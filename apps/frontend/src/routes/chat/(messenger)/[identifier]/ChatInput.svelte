<script lang="ts">
  ...
  let attachments = $derived<z.infer<(typeof attachmentSelectSchema)[] | null>>(
    fileStore.files
  ...
  async function handleSend() {
    ...
    const isUploading = fileStore.files.some((f) => f.status === "uploading");
        ...
        >
          ...
          >
            {#each fileStore.files as entry (entry.id)}
              <Attachment {entry} callback={() => fileStore.remove(entry.id)} />
            {/each}
          </div>
          <button
