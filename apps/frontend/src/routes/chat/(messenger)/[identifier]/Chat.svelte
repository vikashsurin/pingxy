        ...
        >
          ...
          {#if entry.senderId === user.id}
            <div class=" flex flex-col justify-end p-1 max-w-1/2 ml-auto">
              <div class="bg-blue-100 px-3 py-3">
                <span class="font-bold">{entry.messageId}</span>
                <div class="flex flex-col">
                  {@render files(entry.messageId)}
                </div>

                <span>
                  {entry.content}
                </span>
                <span id="meta-data" class="flex gap-2">
                  <span class="text-xs flex opacity-60">
                    <!-- {formatLocalTime(entry.createdAt)} -->
                  </span>
                  <span>
                    {@render receipt(entry.messageId!)}
                  </span>
                </span>
              </div>
            </div>
          {:else}
            ...
            >
              ...
              <span id={entry.messageId.toString()} class="sender">
                {entry.content}
              </span>
