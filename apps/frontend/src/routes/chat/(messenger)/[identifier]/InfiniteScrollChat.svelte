<script lang="ts">
    ...
    // console.log($state.snapshot(uid));
    // console.log($state.snapshot(chatStore.activeConversationId));s

    const LIMIT = $derived(chatStore.LIMIT);

    // const uid = $derived(uid);
    // const chatStore.activeConversationId = $derived(
    //     chatStore.activeConversation?.chatStore.activeConversationId,
    // );
    let unread = $derived(chatStore.unread.get(chatStore.activeConversationId ?? 0));
...
console.log("hurary")
    ...
    async function handleLoadOlder() {
        if (!chatStore.activeConversationId || !uid || !scrollElement) return;
    ...
    async function handleLoadNewer() {
        if (!chatStore.activeConversationId || !uid) return;
    ...
    async function handleNewMessage() {
        ...
        chatStore.unread.delete(chatStore.activeConversationId!);
    }
</script>
...
<div class="overflow-auto relative flex-1 min-h-0 flex-col">
  ...
  {#if virtualStore.jumpToLatest && !chatStore.unread.has(chatStore.activeConversationId!)}
    <button
  ...
  {#if chatStore.unread.has(chatStore.activeConversationId!)}
    <button
      class="absolute z-50 bottom-10 right-1/2 translate-x-1/2"
