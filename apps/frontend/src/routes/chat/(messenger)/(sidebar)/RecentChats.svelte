<script lang="ts">
    ...
    const chats = $derived(
        Array.from(conversationStore.conversationByPartnerId),
    );
    ...
    <li id={id.toString()}>
        <a href="/chat/c_{id}">
            <div
                ...
                style:background-color={`c_${id}` === urlArray.at(-1)
                    ? "orange"
                    : ""}
            >
                <!-- 1. gender icon -->
                ...
                <div class="ml-auto flex items-center justify-end gap-2">
                    ...
                    <span class={` fi fi-${row.country.toLocaleLowerCase()}`}>
                    </span>

                    {@render unreadCount(row)}
                </div>
            </div>
        </a>
    </li>
