...
export const BlockedRepository = {
    insert: async ({ blockerId, blockedId, tx = db }: { blockerId: number, blockedId: number, tx?: DB_TX }) => {
    ...
    selectBlockers: async ({ blockedId, tx = db }: { blockedId: number, tx?: DB_TX }) => {
        const [result] = await tx
            .select()
            .from(blockedUsers)
    ...
    selectAllBlocked: async ({ blockerId, tx = db }: { blockerId: number, tx?: DB_TX }) => {
        const result = await tx
            .select()
            .from(blockedUsers)
    ...
    selectUnique: async ({ blockerId, blockedId, tx = db }: { blockerId: number, blockedId: number, tx?: DB_TX }) => {
        const [result] = await tx
            .select()
            .from(blockedUsers)
            .where(
    ...
    countBlocked: async ({ blockerId, tx = db }: { blockerId: number, tx?: DB_TX }) => {
        const result = await tx
            .select()
            .from(blockedUsers)
            .where(eq(blockedUsers.blockerId, blockerId));
